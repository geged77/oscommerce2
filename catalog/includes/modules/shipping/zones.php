...
// class methods
    ...
    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Activer le module Zones', 'MODULE_SHIPPING_ZONES_STATUS', 'True', 'Voulez-vous activer la livraison par zone ?', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Classe de taxe', 'MODULE_SHIPPING_ZONES_TAX_CLASS', '0', 'Appliquer cette TVA pour ces frais de port.', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Ordre d\'affichage.', 'MODULE_SHIPPING_ZONES_SORT_ORDER', '0', 'Ordre d\'affichage dans la page. Le plus petit en premier.', '6', '0', now())");
      for ($i = 1; $i <= $this->num_zones; $i++) {
        $default_countries = '';
        if ($i == 1) {
          $default_countries = 'US,CA';
        }
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Pays de la Zone " . $i ."', 'MODULE_SHIPPING_ZONES_COUNTRIES_" . $i ."', '" . $default_countries . "', 'Liste des codes ISO2 (2 caractères) de la Zone " . $i . ".', '6', '0', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Table de prix de la Zone " . $i ." ', 'MODULE_SHIPPING_ZONES_COST_" . $i ."', '3:8.50,7:10.50,99:20.00', 'Frais de port de la Zone " . $i . " sur la base du poids maximum de la commande. Exemple: 3:8.50,7:10.50,... poids inférieur ou égal à 3 coûtera 8.50 pour la zone Zone " . $i . ".', '6', '0', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Frais fixes de la Zone " . $i ." ', 'MODULE_SHIPPING_ZONES_HANDLING_" . $i."', '0', 'Frais de manutention ou d\'emballage pour cette zone', '6', '0', now())");
      }
    }
